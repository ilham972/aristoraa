  ...
  return (
    <div className="overflow-x-auto -mx-4 px-4 pb-2">
      <div className="min-w-[640px]">
        {/* Header row */}
        <div className="grid grid-cols-[44px_repeat(7,1fr)] gap-1 mb-1 sticky top-0 z-10">
          <div />
          {DAYS.map((d) => (
            <div
              key={d.num}
        ...
        {rows.map((band, ri) => {
          ...
          return (
            <div
              key={band.start}
              className={cn('grid grid-cols-[44px_repeat(7,1fr)] gap-1 mb-1', isAfterGap && 'mt-2')}
            >
              <div className="text-[9px] text-muted-foreground/70 text-right pr-1 flex items-center justify-end">
                {band.label}
              </div>
              {DAYS.map((d) => {
                ...
                if (disabled) {
                  return <div key={d.num} className="h-12 rounded-md bg-muted/20" />;
                }
                if (!cell) {
                  return (
                    <button
                      ...
                      className="h-12 rounded-md border border-dashed border-border/40 hover:border-primary/50 hover:bg-primary/5 transition-colors text-muted-foreground/0 hover:text-primary/60 text-lg leading-none"
                    >
                      +
                    </button>
                  );
                ...
                // Only the first band of a multi-hour session shows the label;
                // continuation bands render as a plain color block.
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'h-12 px-1.5 py-1 text-left overflow-hidden transition-transform active:scale-95',
                      isHead ? 'rounded-md' : 'rounded-md -mt-2 pt-2.5',
                    )}
                    ...
                    {isHead && (
                      <>
                        ...
                        <div className="flex items-center justify-between mt-0.5">
                          ...
                          <span className="text-[8px] font-medium opacity-90">{fmtLKR(cell.revenue).replace('Rs ', '')}</span>
                        </div>
                      </>
                    )}
                  </button>
