...
import { ClipboardPen, Trophy, Users, Settings, Radio, BarChart3, FileSpreadsheet } from 'lucide-react';
        ...
        {items.map(item => {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                'flex flex-col items-center justify-center gap-0.5 py-1.5 px-1.5 rounded-xl transition-all min-w-[44px]',
                isActive
                  ? 'text-primary'
            ...
            >
              <div className={cn(
                'relative flex items-center justify-center w-9 h-7 rounded-lg transition-all',
                isActive && 'bg-primary/12'
              )}>
                <item.icon
                  className={cn(
                    'w-[19px] h-[19px] transition-all',
