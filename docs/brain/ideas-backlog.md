...
| 5c | **Premium sheet print quality** (founder-requested 2026-06-12). Two root causes found: (a) `fitCropSlot` in pdf.ts stretches EVERY question to 80mm wide regardless of its real size on the textbook page → small questions look huge, big ones tiny; fix = scale crops relative to their source-page proportion so all text prints at one consistent size. (b) Pages upload at pdfjs scale=2 / JPEG 0.85 (~144 DPI) — print needs ~300 DPI; fix = scale 3 + quality 0.9 (re-upload needed to benefit). Plus exam-paper layout polish. | sheets | Brand: Apple-quality feel | M | 4 |
| 5d | **Stem instruction templates.** Textbook stems say "solve ALL the following" but sheets pull one leaf. Fix is one-time per stem (NOT per class): a picker of ~10 canned Tamil instructions ("இதைத் தீர்க்கவும்" etc.) set at crop time + free-text override; AI batch (Claude, few $) for leftovers. | sheets | Personalized education | S | 4 |
