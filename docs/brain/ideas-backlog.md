...
| 5b | **Native-feel performance program** (founder-requested 2026-06-12). Cold open is slow: blank auth screen (auth-layout returns null while Clerk verifies), network-first service worker, ~6 startup queries × 230–780ms Sri Lanka→US RTT. Phases 1–5 SHIPPED 2026-06-12: SW cache-first, skeleton shell, useCachedQuery instant-data layer (home/students/session tabs), optimistic scoring taps, icon diet (1.5MB→0.35MB), dead deps removed, railway map lazy-loaded. Remaining: P6 server-side crop thumbnails (the daily-loop data hog) + P7 scale items. | app shell | Teacher efficiency + brand quality | M | 4 |
