...
2. **Planner** (`planner.ts`, 2309 lines — the heart):
   ...
   - COVERAGE LADDER = THE DEFAULT (2026-07-14, toggle retired): fit is
     ...
     damped ×0.25; too-hard tail (>skill+2) deferred; SR timing untouched.
     Fallback: students.learningMode="consolidation" (student progress page)
     restores Gaussian fit + repeats; daily engineAlerts.ts cron suggests
     flips (weighted fail-rate, 40%/20% hysteresis, consolidationCore.ts).
   - D.3 slots: warm-up OFF-module, Main ON-module (by TRACK, not
     teachingPath), exam-prep module-agnostic past papers. Adaptive sheet
     length; `plannerGaps` surfaces zero-candidate concepts (never blocks).
     D.6 `scoringSnapshot` stores factor values — the "why this question?"
     tooltip reads these, never re-derives.
...
- `/planner` (nav, 2026-07-15) — THE global planning page, 6 tabs; backend
  ...
  Calendar: per-group day-grid to exam (unit/state, Revision slots, cancelled
...
`groupPlan.ts` + pure `convex/lib/groupPlanCore.ts` (tested): a group
session's Main block is ONE identical sheet planned at group level; bookmark
= DERIVED union (groupSheets + members' generatedSheets), never stored.
Skeleton = lesson plan to exam (unit per session, finish-vs-exam verdicts),
CANCELLATION-AWARE (cancelled days skipped, plan reflows). crystallizeUpcoming
writes groupSheets (new = next unseen ladder, textbook then capped past-paper
tail, scanBookExhaustion cron; spiral = unseen Qs from past units, weakest
group-avg R first); session Sheets tab materializes per student via
mainQuestionIdsOverride (PDF/scoring/memory unchanged). Revision dept:
...
carry-over log (tail → "main" served FIRST next crystallize, or "revision"
per-member queue; deletePlanned un-consumes; bookmark never un-seen). Book-gap
honesty: zero-question units = "no-questions" never "done"; exercise deletion
cascades to crops (cleanupCropsForExercise). STARTING POINT: groupPreTaughtUnits
+ setGroupStartingPoint = reversible track-prefix bookmark, no sheet/point/
memory effects; planner paths skip those units. UI: /groups/[id]/plan (runway
strip + unit-SPAN projection) + StartingPointButton.
...
- Coverage forecast advisor (2026-07-14): `coverageForecast.ts` + pure
  ...
  allocation; TERM-AWARE (only units with an UPCOMING exam compete). UI:
  /students/[id]/progress + /planner Forecast rollup (same core).
- Track Progress (2026-07-10): `trackProgress.ts` (metro-line + session
  strip); pure `trackProgressCore.ts` MUST mirror planner pacing. Fixes:
  track units in pool scope; completion credits attempts ≤48h after sheet
  date; getSavedSheet duplicate-row safe.

## Rules for changing the engine
- Read BOTH plan files first; behaviour must match algorithm_plan.md blocks.
- Constants ONLY in config.ts. Scorer stays module-blind (slots are D.3);
  keep every factor exposed. repeatCount is a TEMPORARY stopgap — don't deepen.
